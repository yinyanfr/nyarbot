...
- 🔗 **链接理解（按需）** — 仅在被动触发时由 LLM 按需调用工具抓取；推文走 FxEmbed 2.0，其他链接直抓标题/描述并回退 Tavily
...
│   ├── firestore.ts            # Firestore CRUD（用户、日记、晚安/早安时间戳）
...
| 设置昵称    | 跟她说「叫我XX」                                 |
| 记录记忆    | 跟她说「记住XXX」                                |
| 分享链接    | 直接发链接（被动触发时可按需抓取内容）           |
| 发图片/贴纸 | 直接发送（被动触发时可按需解析媒体）             |
