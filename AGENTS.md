...
- `src/libs/telegram-image.ts` — Telegram image download plus validated original WebM video-sticker download
...
- **Model routing**: Qwen `qwen3.7-flash` is the non-thinking multimodal primary; Telegram WebM video-sticker turns route directly to Gemini 3.5 Flash-Lite; DeepSeek `deepseek-v4-flash` thinking is optional advisor only; DeepSeek V4 Pro is not used.
