...
export interface ProactiveDependencies {
  ...
  containsTwitterStatusUrl: typeof containsTwitterStatusUrl;
  probeGate: typeof probeGate;
  ...
  groupRuntime: typeof groupRuntime;
...
export function createProactiveChecker(overrides: Partial<ProactiveDependencies> = {}) {
  const dependencies: ProactiveDependencies = {
    config,
  ...
  const {
    ...
    containsTwitterStatusUrl: containsStatusUrl,
    probeGate: runProbeGate,
    generateAiTurn: runAiTurn,
    getHistory: loadHistory,
    pushMessage: appendMessage,
    formatHistoryAsContext: formatContext,
    getStickerEmojiByFileId: lookupStickerEmoji,
    groupRuntime: runtime,
    logger: log,
    now,
  } = dependencies;
  const CHECK_INTERVAL_MS = runtimeConfig.proactiveCheckIntervalMs;
  const WINDOW_MS = runtimeConfig.proactiveWindowMs;
  const MESSAGE_DELAY_MS = runtimeConfig.proactiveMessageDelayMs;
  const MAX_FAILURES = runtimeConfig.proactiveMaxFailures;
  ...
  function getCooldownMs(activityCount: number): number {
    if (activityCount >= 7) return runtimeConfig.proactiveCooldownHighMs;
    if (activityCount >= 3) return runtimeConfig.proactiveCooldownMediumMs;
    return runtimeConfig.proactiveCooldownLowMs;
  }
   ...
   */
  function touchBotActivity(): void {
    lastBotMessageTime = now();
  }

  function getProactiveHealthSnapshot(): Readonly<ProactiveHealthSnapshot> {
  ...
  function getNextCheckDelayMs(): number {
    ...
    const boundedFailureCount = Math.min(
      consecutiveFailures,
      Math.max(1, Math.floor(MAX_FAILURES)),
    );
  ...
  async function check(callbacks: ProactiveCallbacks): Promise<void> {
    ...
    try {
      ...
      const history = loadHistory(runtimeConfig.tgGroupId);
      ...
      const recentWindow = history.filter((entry) => entry.timestamp > currentTime - WINDOW_MS);
      const referenceHistory =
        latestBotIndex >= 0 && latestBotHistoryTime <= currentTime - WINDOW_MS
          ? [history[latestBotIndex]!, ...recentWindow]
          : recentWindow;
      const recentHistory = history
        .slice(latestBotIndex + 1)
        .filter((entry) => entry.timestamp > currentTime - WINDOW_MS);
      const candidateHistory = recentHistory.filter((entry) => !containsStatusUrl(entry.text));
      const safeReferenceHistory = referenceHistory.filter(
        (entry) => entry.uid === "bot" || !containsStatusUrl(entry.text),
      );
      ...
      if (!runtime.canRunProactive()) return;
      ...
      if (currentTime - effectiveLastBotMessageTime < cooldown) return;

      const activityRevision = runtime.getActivityRevision();
      const ran = await runtime.runProactiveTurn(async () => {
        ...
        const shouldProceed = await runProbeGate({
        ...
        if (runtime.getActivityRevision() !== activityRevision) {
          log.info({ phase: "after_probe" }, "proactive: conversation changed, skipping");
          return;
        }
        ...
        if (botActivitySnapshot > effectiveLastBotMessageTime) {
          log.info({ phase: "after_probe" }, "proactive: bot activity changed, skipping");
          return;
        }
        ...
        const formattedHistory = formatContext(safeReferenceHistory);
        const formattedCandidates = formatContext(candidateHistory);

        // Collect recent bot messages for human-likeness feedback
        const recentBotMessages = history
          .filter((entry) => entry.timestamp > currentTime - WINDOW_MS && entry.uid === "bot")
          .map((e) => e.text)
          .slice(-5);
        ...
        try {
          ...
          dependencies.clearInterval(typingTimer);
        }

        if (result.action === "dismiss") {
          log.info("proactive: full model chose to dismiss after probe activation");
          await runtime.recordTurn({
            kind: "proactive",
            startedAt: now() - (result.metrics?.latencyMs ?? 0),
            completedAt: now(),
        ...
        const conversationChanged = () =>
          runtime.getActivityRevision() !== activityRevision ||
          lastBotMessageTime !== botActivitySnapshot;
        if (conversationChanged()) {
          log.info({ phase: "before_send" }, "proactive: conversation changed, dropping result");
          return;
        }
        ...
        for (let i = 0; i < result.messages.length; i++) {
          if (i > 0 && conversationChanged()) {
            log.info({ phase: "between_messages" }, "proactive: conversation changed, stopping");
            break;
          }
          ...
          appendMessage(
            ...
            msg.slice(0, MAX_BUFFER_TEXT),
          );
          await runtime.recordBotMessages({ messages: [msg] });

          // Stagger messages to mimic human typing rhythm, but not after the last one
          if (i < result.messages.length - 1) {
            await new Promise((resolve) => dependencies.setTimeout(resolve, MESSAGE_DELAY_MS));
          }
        }
        ...
        if (stickerFileId) {
          if (await callbacks.sendSticker(stickerFileId)) {
            ...
            if (sentMessages.length === 0) {
              const emoji = lookupStickerEmoji(sentStickerFileId) ?? "🐱";
              appendMessage(
                runtimeConfig.tgGroupId,
                "bot",
                runtimeConfig.botUsername,
              ...
              await runtime.recordBotMessages({
                messages: [],
      ...
      log.error(err, `proactive check failed (${consecutiveFailures} consecutive failures)`);
    } finally {
      running = false;
      if (!failed) {
   ...
   */
  function startProactiveChecker(callbacks: ProactiveCallbacks): void {
   ...
   */
  function stopProactiveChecker(): void {
    stopped = true;
    if (timer) {
...
const defaultChecker = createProactiveChecker();
export const touchBotActivity = defaultChecker.touchBotActivity;
export const getProactiveHealthSnapshot = defaultChecker.getProactiveHealthSnapshot;
export const startProactiveChecker = defaultChecker.startProactiveChecker;
export const stopProactiveChecker = defaultChecker.stopProactiveChecker;
