...
- **对话缓冲区**：内存环形缓冲区（每组最多 30 条，每条最多 500 字）。每条用户消息和 bot 回复都会推送。用于 `buildSystemPrompt` 和 `probeGate` 主动插话检查。进程重启后丢失。缓冲区只保留媒体/链接的轻量存在性与引用（包含原始 URL、媒体 file_id），不再预注入 Gemini/Tavily 摘要。
