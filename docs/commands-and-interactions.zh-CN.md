...
| `/stroke`           | 所有人   | 摸 bot 一下，触发被摸到的炸毛反应    |
...
- 部分不是 `send_message` 产生的 bot 输出也会写入对话缓冲区，例如 `/love`、`/shock`、`/stroke`、`/reset`、早安问候、每日自动日记通知。
...
- 如果某天存在转发消息，caption 会明确注明这一点，避免口径误解。
...
| `describeTelegramMedia` | 按需解析当前轮中的 `file_id`/`thumbnail_file_id`（仅被动触发） |
