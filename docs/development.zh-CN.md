...
Bot 通过 `writeDiary` AI 工具记录对话观察笔记，而非事后提取。模型根据对话上下文判断什么值得记录——无规则触发、无频率限制。午夜（UTC+8）使用 DeepSeek v4 Pro 带思考模式将观察汇总为自然的猫娘第一人称日记。生成的日记通过 GitHub Content API 推送到 Hexo 博客供公开阅读。
...
- 所有时区感知的格式化集中在 `src/libs/time.ts`，固定 `Asia/Shanghai` 时区
